	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		include 'includes/dbconn.php';
		 ...
		 $ret=mysqli_query($con,"SELECT count(ID) id2 from vehicle_info where Status='Out'");
		  ...
		  $ret=mysqli_query($con,"SELECT count(ID) as id1 from vehicle_info where VehicleCategory='Bikes'");
		$row=mysqli_fetch_array($ret);  
             
		$ret=mysqli_query($con,"SELECT count(ID) as id2 from vehicle_info where VehicleCategory='Heavy Vehicles'");
		$row2=mysqli_fetch_array($ret); 

		$ret=mysqli_query($con,"SELECT count(ID) as id4 from vehicle_info where VehicleCategory='Light Vehicles'");
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: ["In-Vehicles", "Out-Vehicles"],
        datasets: [{
...
var myChart = new Chart(ctx, {
	type: 'bar',
	data: {
		labels: ["Bikes","Heavy Vehicles", "Light Vehicles"],
		datasets: [{
			backgroundColor: ["#f55d42","#f5c542", "#a14343"],
			data: [<?php echo $row['id1']; ?>, <?php echo $row2['id2']; ?>, <?php echo $row4['id4']; ?>]
		}]
	},
	options: {
        scales: {
